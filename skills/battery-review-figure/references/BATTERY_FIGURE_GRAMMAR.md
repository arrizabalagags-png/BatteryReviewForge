...
本项目的可复现展示图逐项规格见 [SHOWCASE_SPEC.md](SHOWCASE_SPEC.md)，出图前后的实际检查见 [PLOT_QA_CHECKLIST.md](PLOT_QA_CHECKLIST.md)。两份文件是实现配方和质检表；本页仍负责图型选择与科学边界。
...
| 阶梯倍率 | 实际逐圈容量、每阶段倍率、每阶段圈数、返回低倍率数据 | 按时间顺序画容量，并标真实倍率阶段 | 为了图形完整自行添加恢复阶段或对应电压曲线 |
| 软包温度 | 标定后的红外/传感器温度、表面坐标、极耳位置、环境温度、同一色标 | 有几何边界与 °C 色标的温度图；若同源，可附截线 | 把伪彩图当成真实红外图，或让不同时间点各用不同颜色范围 |
| 文献基准图 | 逐点 DOI/来源、指标分母、电芯边界、条件、筛选规则 | 来源可追溯且可比的散点图 | 把演示编号当引文；把不同倍率/载量的数据直接排名 |
