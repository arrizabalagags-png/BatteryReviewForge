...
**新手只需这样说：**“这是我的文件，想做一张论文图。先看看能画什么，还缺什么。”先检查文件并给出可做的图型；作者不用先选技能或填一张长表。投稿图需要核对的电芯构型、容量分母、倍率/电流、温度、载量、电解液量、循环或电压范围，按当前图型逐项看。先完成能核实的部分，只问缺少且会改变图义的条件；不要根据文件名、仪器品牌或截图猜值。

画图前只问一次风格：**“你喜欢哪种配色？看看[预览](../assets/style-preview.svg)；也可以说‘你来选’。”** 预览里有深海蓝绿、玫蓝渐层、珊瑚冰蓝、暖冷对照、期刊极简、清晰对比六种。同一稿件沿用已选风格，不必每画一个面板都重问；作者让你选时，按图的证据角色选并记录。等待选择时继续读数据和核条件，不要把未选风格的图叫作最终交付。
